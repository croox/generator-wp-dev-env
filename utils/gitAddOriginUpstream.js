...
const gitAddOriginUpstream = (generator) =>
	new Promise((resolve) => {
		...
		generator.log('');
		generator.log(chalk.yellow.bold('Add a remote origin and set upstream for main branches'));
		generator.log(
			"It's recommended, but keep in mind, that the remote has to be created before you proceed!"
		);
		...
		prompt([
			...
			.then((answers) => {
				...
				if (origin.length > 0 && origin !== 'skip') {
					simpleGit
						.addRemote('origin', origin)
						.checkout(branchMainName)
						.push(['-u', 'origin', branchMainName])
						.checkout('generated')
						.push(['-u', 'origin', 'generated'])
						.checkout('develop')
