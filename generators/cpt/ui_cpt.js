...
const { startCase, get, find } = require('lodash');
...
const ui_cpt = function (self) {
	...
	const getInitial = (choiceName, prompt) => {
		switch (choiceName) {
			...
			case 'displayName':
				return startCase(
					(prompt.values.singularName || getInitial('singularName')).replace(/-/g, ' ')
				);
	...
	const prompts = [
		{
			...
			validate(value, state, _field) {
				const formValidation = formValidate(value, state, {
					...
					sanitized: ['singularName'],
					notEmpty: ['singularName', 'displayName'],
				});
				...
				if ([...reservedCpts].includes(value.singularName)) {
					const fieldMessage = get(
						find(state._choices, { name: 'singularName' }),
						['message'],
						'singularName'
					);
