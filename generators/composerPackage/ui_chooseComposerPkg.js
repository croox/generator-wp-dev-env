...
const { get, union, has } = require('lodash');
...
const ui_chooseComposerPkg = function (self) {
	...
	[...packageConfigs].forEach((packageConfig) => {
		...
		const newChoice = {
			...
			message: installedPkgs.includes(packageConfig.key)
				? chalk.dim(packageConfig.key + ' (already installed)')
				: chalk.bold(packageConfig.key),
			...(hint.length > 0 && {
				hint: installedPkgs.includes(packageConfig.key) ? chalk.dim(hint) : hint,
			}),
		};

		prompt.choices = [...prompt.choices, newChoice];
	});
